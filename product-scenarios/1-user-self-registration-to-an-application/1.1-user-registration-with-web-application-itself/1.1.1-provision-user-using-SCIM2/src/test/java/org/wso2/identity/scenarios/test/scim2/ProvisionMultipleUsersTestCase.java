...
public class ProvisionMultipleUsersTestCase extends ScenarioTestBase {
    ...
    public void testSCIM2MultipleUsers() throws Exception {
        ...
        data1.put(SCIMConstants.USER_NAME_ATTRIBUTE,SCIMConstants.USERNAME);
        data1.put(SCIMConstants.PASSWORD_ATTRIBUTE, SCIMConstants.PASSWORD);

        JSONObject data2 = new JSONObject();
        data2.put("schemas",schemaUser);
        ...
        rootObject.put("Operations", operations);

        StringEntity entity = new StringEntity(rootObject.toString());
        request.setEntity(entity);

        HttpResponse response = client.execute(request);
        assertEquals(response.getStatusLine().getStatusCode(), 200, "Users bulk schema has not been created successfully");

        Object responseObj = JSONValue.parse(EntityUtils.toString(response.getEntity()));
        EntityUtils.consume(response.getEntity());
        assertTrue(((JSONObject) ((JSONArray) ((JSONObject) responseObj).get("Operations")).get(0)).get("status").toString().contains("201"));
        assertTrue(((JSONObject) ((JSONArray) ((JSONObject) responseObj).get("Operations")).get(1)).get("status").toString().contains("201"));

        String userID1 = (((JSONObject) ((JSONArray) ((JSONObject) responseObj).get("Operations")).get(0)).get("location").toString().split("/"))[5];
        String userID2 = (((JSONObject) ((JSONArray) ((JSONObject) responseObj).get("Operations")).get(1)).get("location").toString().split("/"))[5];
    ...
    private void testDeleteUser(String bulkUserId) throws Exception {
        ...
        String userResourcePath =
                getDeploymentProperties().getProperty(IS_HTTPS_URL) + SCIMConstants.SCIM2_USERS_ENDPOINT + "/" + bulkUserId;
